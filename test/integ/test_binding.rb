...
# Integration tests for WebDAV BIND (RFC 5842) operations.
#
# Converted from DAViCal's regression test suite:
#   refs/davical/testing/tests/binding/
#
# Original .test/.result fixtures preserved in:
#   test/integ/fixtures/binding/
#
...
class TestBinding < Caldav::IntegrationTest
  i_suck_and_my_tests_are_order_dependent!

  FIXTURES = File.expand_path('fixtures/binding', __dir__)

  # Tests to skip: 1310 is a Perl/WWW::Mechanize browser test, not CalDAV.
  # 1300/1301/1302 require external URL fetching (STATIC= directive).
  SKIP_TESTS = %w[
    1310-BIND-external-webui
    1300-BIND-external
    1301-GET-fetch-external
    1302-PROPFIND-external
  ].freeze

  # Shared state across ordered tests (ticket IDs, etc.)
  ...
  Dir[File.join(FIXTURES, '*.test')].sort.each do |test_file|
    ...
    next if SKIP_TESTS.include?(name)

    define_method("test_#{name.tr('-', '_')}") do
      run_binding_test(name)
    end
  end
  ...
  # ---------------------------------------------------------------------------
  def run_binding_test(name)
    req = parse_test_file(name)
    expected = parse_result_file(name)
    return if expected.nil? # no .result file
    ...
    # Capture ticket ID from MKTICKET responses for later BIND tests
    @@state[:ticket] = resp.headers['Ticket'] if req[:method] == 'MKTICKET' && resp.headers['Ticket']
  ...
  def parse_test_file(name)
    ...
    lines.each do |line|
      ...
      when /^HEADER=Ticket:\s*(.+)/i
        ...
        # Substitute ##ticket## with captured ticket from earlier MKTICKET
      ...
      when /^HEADER=User-Agent:\s*(.+)/i
        result[:headers]['HTTP_USER_AGENT'] = Regexp.last_match(1).strip
      when /^REPLACE=(.)(.*)\1(.*)\1$/
        # REPLACE=!pattern!replacement! or REPLACE=/pattern/replacement/
      ...
      when 'HEAD', /^DOSQL/, /^STATIC=/, /^HEADER=/, ''
        # Skip: HEAD marker, DOSQL, STATIC directives, unknown headers, blank lines
        next
      end
    end
  ...
  def parse_result_file(name)
    ...
    if content.start_with?('HTTP/')
      # First line is HTTP status line: "HTTP/1.1 201 Created"
      first_line, rest = content.split("\n", 2)
      status = first_line[%r{HTTP/\d+\.\d+\s+(\d+)}, 1].to_i

      # Body starts after the first blank line (end of headers)
      _headers, body = rest.split("\n\n", 2)
      body ||= ''
    else
      # No HTTP headers -- raw XML body, implicit 207 Multi-Status
      status = 207
      body = content
    end
